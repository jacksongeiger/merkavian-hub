...
export function IframePanel({ name, src, loadTimeoutMs = 8000 }: Props) {
    ...
    >
      ...
      <iframe
        ...
        style={{
          ...
          height: "100%",
          border: "none",
          background: "var(--cds-color-backgroundSurface, transparent)",
...
function IframeFallback({ name, src }: { name: string; src: string }) {
  return (
    ...
    >
      <VStack gap={3} alignItems="center" style={{ maxWidth: 480, textAlign: "center" }}>
        ...
        <Button as="a" href={src} target="_blank" rel="noopener noreferrer" variant="secondary">
          Open in new tab
        </Button>
      </VStack>
    </Box>
